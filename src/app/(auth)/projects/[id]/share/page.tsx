...
export default function ProjectSharePage({
  ...
  async function handleDelete(tokenId: string) {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/share?tokenId=${tokenId}`, {
        method: "DELETE",
      })
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          <h1 className="text-2xl font-bold text-icc-violet">Liens de partage</h1>
          <p className="text-gray-700 mt-1">
            Créez des liens pour partager ce projet
          </p>
        </div>
        {!showForm && (
      ...
      {loading ? (
        <div className="text-center py-12">
          <div className="w-8 h-8 mx-auto border-2 border-icc-violet border-t-transparent rounded-full animate-spin" />
        </div>
      ) : tokens.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-700 mb-6">
              Créez un lien pour partager ce projet avec des réviseurs ou
              l&apos;équipe média.
            </p>
            {!showForm && (
                ...
                <div className="flex flex-col sm:flex-row justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {isExpired(token.expiresAt) && (
                        <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-red-100 text-icc-rouge">
                          Expiré
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className="text-icc-rouge hover:text-icc-rouge hover:bg-red-50"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
