...
export default function GalleryPage() {
  ...
  async function downloadPhoto(photoId: string) {
    ...
    try {
      const res = await fetch(`/api/gallery/${token}/photo/${photoId}`)
      const response = await res.json()
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-icc-violet mx-auto" />
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-4xl mx-auto px-4 py-12 text-center">
          ...
          <h2 className="text-lg font-medium text-gray-900 mb-2">Aucune photo disponible</h2>
          <p className="text-gray-600">
            {data.onlyApproved
              ? "Les photos validées apparaîtront ici une fois la validation terminée."
              : "Aucune photo n'a encore été uploadée pour cet événement."}
          </p>
        </div>
      </div>
    )
              ...
              >
                ...
                <div className="p-2">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 py-1.5 text-xs font-medium text-icc-violet hover:bg-icc-violet hover:text-white rounded transition-colors disabled:opacity-50"
